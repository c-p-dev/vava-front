...
mainAngular.directive('restrictTo', function() {
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            var exclude = /Backspace|Enter|decimal point|Tab|Delete|Del|ArrowUp|Up|ArrowDown|Down|ArrowLeft|Left|ArrowRight|Right/;
...
mainAngular.controller("mc", function($window, $scope, $templateCache, $compile, $http, wss) {	
	...
	wss.con();
		...
		$scope.fmid =document.getElementById("mid").value; 
		$scope.fsid =document.getElementById("sid").value; 
		$scope.ftab =document.getElementById("tab").value; 
		
		$scope.fmain = function () {
			
			if($scope.fmid.length > 1){
				
				if($scope.ftab == '1' || $scope.ftab == '3' ){
					$scope.loadPop($scope.fsid,$scope.fmid);				
				} else if($scope.ftab == '2'){				
					$scope.goMatchMarket('center_market.html',$scope.fsid,$scope.fmid);
				} 
    ...
    console.log("$scope.bet:"+$scope.bet.length);
  }
 ...
 if($scope.ISLG){
 	...
 	$scope.loadLiveMatchInit = function() {		
 	...
 	$scope.loadPreMatchInit = function() {		
 	...
 	$scope.loadLiveMatchInit();
	$scope.loadPreMatchInit();  
	 ...
	 $scope.goLiveMatch = function (left,center) {	  
	 	//console.log("goLiveMatch : " + center);	
	  	$scope.loadLiveMatchInit();
	  	$scope.loadPreMatchInit();  
	  	//preMatchCnt();  	
	  	//console.log("$scope.PM.length:"+$scope.PM.length);
	  	...
	  	islive = true;
			...
			$scope.tabMId = "0";
			$scope.tabSId ="0";  	 	
	    $scope.centerTab = "center_live.html";
	    $scope.leftTab = left;
	    mode = "LiveMatch";
	  }

	 $scope.goPreMatch = function (left,center) {	    	
	 	console.log("goPreMatch");	 	
	 	$scope.centerTab = "center_pre.html";
...
*/ 
  ...
  $http({
		...
		//console.log("$scope.validMKTypeId[B]");
 ...
 function subLoadPreMatch(mid) {				
	...
	$scope.getMarketbyMatchId = function(mid) {	
	   ...
	   $http({
			...
			if(data) {
				//console.log("getMarketbyMatchId");
				//console.log(data);
				$scope.MK = [];
				var se_tmp = [];
	   ...
	   $http({
			...
			/*
				...
				if(data[0].Re[0] == 'undefined' || data[0].Re.length<1){
					//ret=ret+1;					
					//var myVar = setTimeout(subLoadLiveMatch(mid), 5000);
			...
			*/
				...
				if(mode != "PreMatch"){
					...
					if(data[0].Mk !== undefined && data[0].Mk !== null && data[0].Mk.length > 0){
						for (var j = 0; j < data[0].Mk.length ; j++) {					
							console.log("data[0].Mk[j] : " + data[0].Mk[j]);	
			    ...
			    for(var i = data[0].Se.length - 1; i >= 0; i--){						
					...
					console.log("data[0].Mt[0].SId : " + data[0].Mt[0].SId);
					console.log("data[0].Mt[0].SN : " + data[0].Mt[0].SN);
					console.log("data[0].Mt[0].RId : " + data[0].Mt[0].RId);
					console.log("data[0].Mt[0].RN : " + data[0].Mt[0].RN);
					console.log("data[0].Mt[0].CId : " + data[0].Mt[0].CId);
					console.log("data[0].Mt[0].CN : " + data[0].Mt[0].CN);
					console.log("data[0].Mt[0].Id : " + data[0].Mt[0].Id);
					console.log("data[0].Mt[0].HT : " + data[0].Mt[0].HT);
			...
			console.log("data[0].Re[0]:" +data[0].Re[0]);
			console.log(data[0].Re[0]);
...
//console.log("mode:"+mode);
  ...
  function sub_loadGetMatch(res) {		  	  	
			...
			if(mode == "LiveMatch"){ 				
		  	...
		  	angular.forEach($scope.LS, function(Ls_obj,i2) {
				...
				if(nlm && obj.IsVisible ){
					...
					console.log("sub_loadGetMatch:[tabDt] = "+$scope.tabDt);	
	...
	//$scope.tw = angular.element(document.getElementById("twidth"))[0].clientWidth;
	
	/*
 	$scope.getWidth = function(){	
 		
 		//var count_table = $(".sports_title_table_1").length; 
 		//var count_table = angular.element(document.getElementById("sports_title_table_1")).getBoundingClientRect();
 		//var count_table = document.getElementById("sports_title_table_1").clientWidth; 		
 		//var count_table =  document.querySelectorAll("sports_title_table_1")[0].clientWidth;
 		
 		$scope.tw = angular.element(document.getElementById("twidth"))[0].clientWidth;
 		$scope.tw2 = $scope.tw/2-20;
 		$scope.tw3 = angular.element(document.getElementById("twidth"))[0].clientWidth/3;
 		$scope.tw4 = angular.element(document.getElementById("twidth"))[0].clientWidth/4;
 		
 		console.log("twidth : " + $scope.tw);
		...
		//var selWidth ="";
		//if(cnt == 3 ) 	selWidth = tw/3-13; // $scope.selWidth = '399px';
		//else if(cnt == 2 ) 	selWidth = tw/2-12; // $scope.selWidth = '605px';
		//else if(cnt == 5 ) 	selWidth = tw/5-15; //(1200/5*100); ///100; ///cnt+1; // $scope.selWidth = '605px';
		//else selWidth = tw/4-14; //$scope.selWidth = '296px';
			
		//console.log("selWidth : " + selWidth*cnt);	
		//return selWidth;
		//return tw/3-40;
		return $scope.tw;
...
*/
  ...
  $scope.selectSport = function (tab,sid) {	//sports tab    	
	...
	$scope.us_mkt = function(mid,mkid) {		
		...
		//console.log("mid:"+ mid);
		//console.log("mkid:"+ mkid);
		
		//$scope.savedFavM = [];		
		
		$('#'+mkid).toggleClass("fa-star fa-star-o");		
		
		//console.log(localStorage.getItem('SFM'));
		...
		//console.log($scope.savedFavM);
			
		localStorage.setItem('SFM',JSON.stringify($scope.savedFavM));		
		//console.log("JSON.parse(JSON.stringify($scope.savedFavM)");
		
		$scope.savedFavM = JSON.parse(localStorage.getItem('SFM'));
		
		//console.log(JSON.parse(localStorage.getItem('SFM')));
		}
	};
...
/*
 function getUbal() {		
	...
	if(data) {	
		...
		console.log("getUbal:"+data.trim());
		
		return data.trim();
		
			//$scope.ubal = data.trim();
	}	
