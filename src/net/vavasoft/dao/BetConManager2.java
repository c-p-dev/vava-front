...
public class BetConManager2 {
	...
	public BetConManager2() {
		...
		try {
			...
			ds = (DataSource) envContext.lookup("jdbc/vava");
	...
	public StringBuffer getLiveMatches() throws SQLException {
		...
		String query = " SELECT '{\"Id\":\"'+CAST(A.match_id as varchar(10))+'\",\"CId\":\"'+CAST(A.compet_id as varchar(10))+'\",\"CN\":\"'+CNAME+'\","
		        ...
		        +
				// " \"LiveStatus\":\"'+CAST(A.live_stat as
				// varchar(1))+'\",\"MatchStatus\":\"'+CAST(A.match_stat as
				// varchar(1))+'\","+
				// " \"IsLiveAvailable\":\"'+CASE WHEN live_stat = 0 THEN 'true'
				// ELSE 'false' END+'\","+
				// " \"IsVisible\":'+CASE WHEN visible ='T' THEN 'true' ELSE
				// 'false' END+', "+
		        "\"Sus\":'+CASE WHEN suspend ='T' THEN 'true' ELSE 'false' END+'," +
				// " \"IsLive\":\"'+CASE WHEN live ='T' THEN 'true' ELSE 'false'
				// END+'\","+
				// " \"IsStarted\":\"'+CASE WHEN match_start ='T' THEN 'true'
				// ELSE 'false' END+'\", "+
		        "\"HT\":\"'+home_name+'\"," + "\"AT\":\"'+away_name+'\"},' AS match  " + " FROM bc_match A ,"
		...
		String query4 = " SELECT  '{\"Id\":'+CAST(A.MARKET_ID as varchar(10))+',\"MId\":'+CAST(B.MATCH_ID as varchar(10))+', "
				...
				// " AND A.visible='T' AND A.suspend = 'F' and home_name is not
				// null and away_name is not null"+
				// " AND A.visible='T' and home_name is not null and away_name
				// is not null"+
		...
		try {
			...
			System.out.println("getLiveMatches : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getLiveMatches] : " + e.getMessage());
	...
	public StringBuffer subLoadLiveMatch(String mid) throws SQLException {
		...
		String query3 = " DECLARE @list varchar(MAX) "
		        ...
		        + " FROM BC_MARKET A,  BC_MATCH B "
		        + " WHERE A.MATCH_ID = B.MATCH_ID and home_name is not null and away_name is not null AND MARKET_TYPE_ID in "
		        + " (SELECT splitdata FROM SplitString(@list,',')) " + " AND B.match_id='" + mid
		        + "' AND B.visible='T'  AND B.booked='T' " + " AND A.visible='T' AND A.suspend = 'F' ";

		String query4 = " SELECT  '{\"Id\":'+CAST(A.MARKET_ID as varchar(10))+',\"MId\":'+CAST(B.MATCH_ID as varchar(10))+',"
		        + "\"Sus\":'+CASE WHEN A.suspend ='T' THEN 'true' ELSE 'false' END+',"
		        + "\"Vis\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE 'false' END+'},' as result "
		        + " FROM BC_MARKET A ,BC_match b " + " WHERE B.match_id='" + mid
		        + "' AND B.visible='T'  AND B.booked='T'  "
		...
		String query5 = " SELECT  '{\"Id\":\"'+CAST(C.SEL_ID as varchar(10))+'\",\"MId\":'+CAST(B.MATCH_ID as varchar(10))+',\"MkId\":\"'+CAST(C.Market_Id as varchar(10))+'\", "
		        ...
		        + "  \"Sus\":'+CASE WHEN A.suspend ='T' THEN 'true' ELSE 'false' END+', "
		        + " \"Vis\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE 'false' END+'},' as selection "
		        + " FROM BC_MARKET A ,BC_match b, bc_selection c " + " WHERE  B.match_id='" + mid
		        + "'  AND B.visible='T'  AND B.booked='T' "
		        + " AND A.visible='T' and a.market_id = c.market_id and home_name is not null and away_name is not null"
		        + " and a.MATCH_ID = b.MATCH_ID AND (MARKET_TYPE_ID ='377' or MARKET_TYPE_ID='399' or MARKET_TYPE_ID='2434' or MARKET_TYPE_ID='4475' "
		        + " OR MARKET_TYPE_ID='5488' OR MARKET_TYPE_ID='5498'   OR MARKET_TYPE_ID='8774' OR MARKET_TYPE_ID='6544' "
	...
	public StringBuffer getPreMatchCnt() throws SQLException {
		...
		// System.out.println("[getPreMatches] :" + query);
		...
		try {
			...
			// System.out.println("[getPreMatches] : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getPreMatches] : " + e.getMessage());
	...
	public StringBuffer getPreMatches() throws SQLException {
		...
		String query = " SELECT '{\"Id\":\"'+CAST(A.match_id as varchar(10))+'\",\"CId\":\"'+CAST(A.compet_id as varchar(10))+'\",\"CN\":\"'+CNAME+'\", "
		        ...
		        + " FROM BC_COMPETITION A LEFT OUTER JOIN BC_TRANS B ON A.NAMEID = B.NAMEID) D "
		        + " WHERE A.sport_id = B.SPORT_ID  AND A.region_id = C.REGION_ID AND A.compet_id = D.COMPET_ID " +
				// " AND live='T' and a.live_stat='1' AND a.visible='T' AND
				// a.booked='T' ";
	...
	public StringBuffer getMatchInfobyCompetition(String cid) throws SQLException {
		...
		String query2 = " DECLARE @list varchar(MAX) "
		        ...
		        + " \"V\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE 'false' END +',"
		        + " \"S\":'+CASE WHEN A.suspend ='T' THEN 'true' ELSE 'false' END +'},' as market "
		        + " FROM BC_MARKET A,  BC_MATCH B " + " WHERE A.MATCH_ID = B.MATCH_ID AND MARKET_TYPE_ID in "
		        + " (SELECT splitdata FROM SplitString(@list,',')) " + " AND B.compet_id='" + cid
		...
		String query3 = " SELECT  '{\"Id\":'+CAST(A.MARKET_ID as varchar(10))+',\"MId\":'+CAST(B.MATCH_ID as varchar(10))+', "
		        + "	\"Sus\":'+CASE WHEN A.suspend ='T' THEN 'true' ELSE 'false' END+'},' " +
				// " \"IsVisible\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE
				// 'false' END+'},' as result "+
		        " as result " + " FROM BC_MARKET A ,BC_match b " + " WHERE  B.compet_id='" + cid
		        + "' AND B.match_stat='0' AND B.visible='T'  AND B.booked='T'  " + " AND A.visible='T' "
		        + " and convert(char(8),match_date,112) >= convert(char(8),getdate(),112) "
		        + " and convert(char(8),match_date,112) <= convert(char(8),getdate()+7,112) "
		        + " and home_name is not null and away_name is not null"
		        + " and a.MATCH_ID = b.MATCH_ID AND (MARKET_TYPE_ID ='377' or MARKET_TYPE_ID='399' or MARKET_TYPE_ID='2434' or MARKET_TYPE_ID='4475' "
		...
		String query4 = " SELECT  '{\"Id\":\"'+CAST(C.SEL_ID as varchar(10))+'\",\"MId\":'+CAST(B.MATCH_ID as varchar(10))+',\"MkId\":\"'+CAST(C.Market_Id as varchar(10))+'\", "
		        ...
		        + "  \"Sus\":'+CASE WHEN A.suspend ='T' THEN 'true' ELSE 'false' END+'},' " +
				// " \"IsVisible\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE
				// 'false' END+'},' as selection "+
		        " as selection " + " FROM BC_MARKET A ,BC_match b, bc_selection c " + " WHERE  B.compet_id='" + cid
		        + "' AND B.match_stat='0' AND B.visible='T'  AND B.booked='T' "
		...
		// System.out.println("[getMatchInfobyCompetition] query: " + query);
		// System.out.println("[getMatchInfobyCompetition] query2: " + query2);
		// System.out.println("[getMatchInfobyCompetition] query3: " + query3);
		// System.out.println("[getMatchInfobyCompetition] query4: " + query4);
		...
		try {
			...
			// System.out.println("getMatchInfobyCompetition : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getMatchInfobyCompetition] : " + e.getMessage());
	...
	public StringBuffer getMatchInfobyMatchId(String mid) throws SQLException {
		...
		// System.out.println("[getMatchInfobyMatchId] query: " + query);
		...
		try {
			...
			// System.out.println("getMatchInfobyMatchId : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getMatchInfobyMatchId] : " + e.getMessage());
	...
	public StringBuffer getMarketbyMatchId(String mid) throws SQLException {
		...
		String query1 = "DECLARE @list varchar(MAX)  "
		        ...
		        + " as market FROM bc_Market " + " where MATCH_ID = '" + mid
		        + "' and MARKET_TYPE_ID in (SELECT splitdata FROM SplitString(@list,',')) and  VISIBLE='T' And Suspend = 'F' ";
		...
		// System.out.println("[getMarketbyMatchId] : " + query1);
		// System.out.println("[getMarketbyMatchId] : " + query2);
		...
		try {
			...
			System.out.println("[getMarketbyMatchId] : " + e.getMessage());
	...
	public StringBuffer getValidMarketType() throws SQLException {
		...
		try {
			...
			// System.out.println("[getValidMarketType] : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getValidMarketTtype] : " + e.getMessage());
	...
	public StringBuffer getFavMatch(String sid, String uid) throws SQLException {
		...
		String query = " SELECT '{\"Id\":\"'+CAST(A.match_id as varchar(10))+'\",\"CN\":\"'+CNAME+'\", "
		        + " \"SId\":\"'+CAST(A.sport_id as varchar(10))+'\",\"SN\":\"'+SNAME+'\",\"RId\":\"'+CAST(A.region_id as varchar(10))+'\",\"RN\":\"'+RNAME+'\", "
		        + "  \"Dt\":\"' + CAST( MATCH_DATE AS VARCHAR(30)) +'\",\"HT\":\"'+home_name+'\",  "
		        + " \"AT\":\"'+away_name+'\"},' AS fav  " + " FROM bc_match A ,"
		        + " (SELECT SPORT_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE SPORT_NAME END  AS SNAME "
		        + " FROM bc_sport A LEFT OUTER JOIN BC_TRANS B ON A.SPORT_NAMEID = B.NAMEID) B,"
		        + " (SELECT REGION_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE REGION_NAME END AS RNAME "
		        + " FROM  BC_REGION A LEFT OUTER JOIN BC_TRANS B ON A.REGION_NAMEID = B.NAMEID) C,"
		        + " (SELECT COMPET_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE COMPET_NAME END AS CNAME "
		        + " FROM BC_COMPETITION A LEFT OUTER JOIN BC_TRANS B ON A.NAMEID = B.NAMEID) D, " + " MATCH_FAVOR G "
		        + " WHERE A.sport_id = B.SPORT_ID  AND A.region_id = C.REGION_ID AND A.compet_id = D.COMPET_ID "
		        + " AND A.MATCH_ID = G.MATCH_ID AND G.USE_FLAG='T'AND (A.match_stat = '0' OR A.match_stat = '1') AND G.USER_ID ='"
		        + uid + "'";

		// System.out.println("[getFavMatch] query :" + query);
		...
		try {
			...
			// System.out.println("[getFavMatch] sb :" + sb.toString());

			con.close();
			// System.out.println("[getValidMarketType] : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getFavMatch] : " + e.getMessage());
	...
	public StringBuffer setFavMatch(String sid, String uid, String mid, String f) throws SQLException {
		...
		// System.out.println("[setFavMatch] query3 :" + query3);
		...
		try {
			...
			// System.out.println("[setFavMatch] r_up : "+r_up);
			...
			if (r_up < 1) {
				...
				// System.out.println("[setFavMatch] r_ins : "+r_ins);
				pstmt.close();
			...
			// System.out.println("[setFavMatch] sb :" + sb.toString());

			con.close();
			// System.out.println("[getValidMarketType] : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[setFavMatch] : " + e.getMessage());
	...
	public StringBuffer getMarketbyMarketId(String mkid) throws SQLException {
		...
		String query1 = "DECLARE @list varchar(MAX)  "
		        ...
		        + " \"Sus\":'+CASE WHEN Suspend ='T' THEN 'true' ELSE 'false' END+',"
		        + " \"Nm\":\"'+replace(replace(replace(market_name,'{sw}',SEQUENCE),'{pw}',POINT_SEQ),'{s}',SEQUENCE) +'\"' "
		        + " as market FROM bc_Market " + " where Market_Id = '" + mkid
		        + "' and MARKET_TYPE_ID in (SELECT splitdata FROM SplitString(@list,',')) "; // and
		                                                                                     // VISIBLE='T'
		                                                                                     // And
		                                                                                     // Suspend
		                                                                                     // =
		...
		String query2 = " DECLARE @list varchar(MAX) "
		        ...
		        + " as sel from bc_selection a, (SELECT MARKET_ID,HANDICAP FROM bc_market WHERE MARKET_TYPE_ID in (SELECT splitdata FROM SplitString(@list,',')) "
		        +
				// " and Market_Id = '"+mkid+"' AND suspend = 'F' AND
				// visible='T' ) B "+
		        " and Market_Id = '" + mkid + "' ) B " + " where a.MARKET_ID = B.MARKET_ID ";

		// System.out.println("[getMarketbyMarketId] : " + query1);
		// System.out.println("[getMarketbyMarketId] : " + query2);
	...
	public StringBuffer getStatusbyMatchId(String mid, String sid) throws SQLException {
		...
		// System.out.println("[getStatusbyMatchId] : " + query1);
		...
		try {
			...
			System.out.println("[getStatusbyMatchId] : " + e.getMessage());
	...
	public List<MatchResultBean> getAllMatchResult() throws SQLException {
		...
		query1 = " select top 100 convert(char(8),match_date,112) MT,match_date MDT,A.sport_id as SId,SNAME as SN,A.region_id as RId,RNAME as RN,A.compet_id as CId,CNAME as CN,home_name as HN,away_name as AN,a.market_id as MKID,a.match_id as MId,E.score as SC,F.cnt as CNT "
		        ...
		        + " a.match_id = e.match_id and " + " a.match_id = F.match_id and " + " e.match_id = F.match_id and "
		...
		System.out.println("[getAllMatchResult] : " + query1);
		...
		try {
			...
			while (rs.next()) {
				...
				// mrb.setMdate2(rs.getString("MDT"));
			...
			System.out.println("[getAllMatchResult] : " + e.getMessage());
	...
	public List<MarketListBean> getMarketList(String mid) throws SQLException {
		...
		query1 = " DECLARE @list varchar(MAX) " + " SELECT @list = JSON FROM BC_JSON WHERE KEYNAME='MARKET_TYPE' "
		        + " select market_type_id,market_id, match_id, market_name FROM bc_Market  where MATCH_ID = '" + mid
		        + "' and MARKET_TYPE_ID in (SELECT splitdata FROM SplitString(@list,',')) ";

		// System.out.println("[getMarketList] : " + query1);
	...
	public List<SelectionBean> getSelectionList(String mid) throws SQLException {
		...
		// System.out.println("[getSelectionList] : " + query1);
		...
		try {
			...
			System.out.println("[getSelectionList] : " + e.getMessage());
	...
	public List<MatchListBean> getFavoriteMatch() throws SQLException {
		...
		query1 = "  select top 10 convert(char(19),match_date,120) MT,A.sport_id as SId,SNAME as SN,A.region_id as RId,RNAME as RN,A.compet_id as CId,CNAME as CN,"
		        + " home_name as HN,away_name as AN,a.match_id as MId " + " from bc_match a, "
		        + " (SELECT SPORT_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE SPORT_NAME END  AS SNAME FROM bc_sport A LEFT OUTER JOIN BC_TRANS B ON A.SPORT_NAMEID = B.NAMEID) B, "
		        + " (SELECT REGION_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE REGION_NAME END AS RNAME FROM  BC_REGION A LEFT OUTER JOIN BC_TRANS B ON A.REGION_NAMEID = B.NAMEID) C, "
		        + " (SELECT COMPET_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE COMPET_NAME END AS CNAME FROM BC_COMPETITION A LEFT OUTER JOIN BC_TRANS B ON A.NAMEID = B.NAMEID) D "
		        + " where  A.sport_id = B.SPORT_ID  AND A.region_id = C.REGION_ID AND A.compet_id = D.COMPET_ID and  popular='T' "
		        + " and a.match_stat='0' and a.match_date >= getdate()" + " order by match_date ";

		// System.out.println("[getFavoriteMatch] : " + query1);
		...
		try {
			...
			System.out.println("[getFavoriteMatch] : " + e.getMessage());
	...
	public List<DepositListBean> getDepositList() throws SQLException {
		...
		// System.out.println("[getDepositList] : " + query1);
	...
	public StringBuffer getMainLiveMatch() throws SQLException {
		...
		String query1 = " SELECT '{\"Id\":\"'+CAST(A.match_id as varchar(10))+'\",\"CId\":\"'+CAST(A.compet_id as varchar(10))+'\",\"CN\":\"'+CNAME+'\","
		        + "\"SId\":\"'+CAST(A.sport_id as varchar(10))+'\",\"SN\":\"'+SNAME+'\",\"RId\":\"'+CAST(A.region_id as varchar(10))+'\",\"RN\":\"'+RNAME+'\","
		        + "\"Sus\":'+CASE WHEN suspend ='T' THEN 'true' ELSE 'false' END+'," + "\"HT\":\"'+home_name+'\","
		        + "\"AT\":\"'+away_name+'\"},' AS match  " + " FROM bc_match A ,"
		        + " (SELECT SPORT_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE SPORT_NAME END  AS SNAME "
		        + " FROM bc_sport A LEFT OUTER JOIN BC_TRANS B ON A.SPORT_NAMEID = B.NAMEID) B,"
		        + " (SELECT REGION_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE REGION_NAME END AS RNAME "
		        + " FROM  BC_REGION A LEFT OUTER JOIN BC_TRANS B ON A.REGION_NAMEID = B.NAMEID) C,"
		...
		String query2 = " DECLARE @list varchar(MAX) "
		        ...
		        + " (SELECT splitdata FROM SplitString(@list,',')) " +
				// " AND B.live='T' and B.live_stat='1' AND B.visible='T' AND
				// B.booked='T' "+
		        " AND A.visible='T' AND A.suspend = 'F' ";
		...
		String query3 = " SELECT  '{\"Id\":'+CAST(A.MARKET_ID as varchar(10))+',\"MId\":'+CAST(B.MATCH_ID as varchar(10))+',"
		        ...
		        + "\"Vis\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE 'false' END+'},' " + " as result "
		        + " FROM BC_MARKET A , (select * from BC_MATCH where live='T' and live_stat='1' AND visible='T' and booked='T' and home_name is not null and away_name is not null ) b "
		        + " WHERE a.MATCH_ID = b.MATCH_ID " + // and a.market_id =
		                                              // b.market_id";
		...
		String query4 = " SELECT  '{\"Id\":\"'+CAST(C.SEL_ID as varchar(10))+'\",\"MId\":'+CAST(B.MATCH_ID as varchar(10))+',\"MkId\":\"'+CAST(C.Market_Id as varchar(10))+'\","
		        ...
		        +
				// " \"Sus\":'+CASE WHEN A.suspend ='T' THEN 'true' ELSE 'false'
				// END+', "+
				// " \"Vis\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE 'false'
				// END+'},' as selection "+
		        ...
		        +
				// " WHERE B.live='T' and B.live_stat='1' AND B.visible='T' AND
				// B.booked='T' "+
				// " AND A.visible='T' and a.market_id = c.market_id and
				// b.market_id = c.market_id and home_name is not null and
				// away_name is not null"+
		        " WHERE a.MATCH_ID = b.MATCH_ID and a.market_id = c.market_id " + // b.market_id
		                                                                          // =
		                                                                          // c.market_id
		                                                                          // ";
		        " and (MARKET_TYPE_ID ='377' or MARKET_TYPE_ID='399' or MARKET_TYPE_ID='2434' or MARKET_TYPE_ID='4475' "
		...
		// " WHERE B.live='T' and B.live_stat='1' AND B.visible='T' AND
		// B.booked='T' "+
		// " AND A.visible='T' and a.market_id = c.market_id and b.market_id =
		// c.market_id and home_name is not null and away_name is not null"+
		...
		try {
			...
			System.out.println("getMainLiveMatch : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getMainLiveMatch] : " + e.getMessage());
	...
	public StringBuffer getMainPreMatch(String sid, String min) throws SQLException {
		...
		String query1 = " SELECT top 10  '{\"Id\":\"'+CAST(b.match_id as varchar(10))+'\",\"Dt\":\"'+convert(char(5),match_date,108)+'\",\"CId\":\"'+CAST(b.compet_id as varchar(10))+'\",\"CN\":\"'+CNAME+'\", "
		        ...
		        + " \"Sus\":'+CASE WHEN suspend ='T' THEN 'true' ELSE 'false' END+'," + " \"HT\":\"'+home_name+'\",  "
		        + " \"AT\":\"'+away_name+'\"},' AS match  " + " FROM bc_match B ,"
		        + " (SELECT SPORT_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE SPORT_NAME END  AS SNAME "
				...
				// " AND live='T' and a.live_stat='1' AND a.visible='T' AND
				// a.booked='T' and A.home_name is not null and A.away_name is
				// not null";
		        " and b.match_stat='0' " + add_query1 + " and b.match_date >= getdate() " + add_query2
		        + " and home_name is not null and away_name is not null order by match_date ";
		...
		String query2 = " DECLARE @list varchar(MAX) "
		        ...
		        + "	\"V\":'+CASE WHEN A.visible ='T' THEN 'true' ELSE 'false' END +', "
		        + "	\"S\":'+CASE WHEN A.suspend ='T' THEN 'true' ELSE 'false' END +'},' as market "
		        + " FROM BC_MARKET A,  ( select top 10  * from BC_MATCH b where match_stat='0' " + add_query1
		        + " and match_date >= getdate() " + add_query2
		        + " and home_name is not null and away_name is not null order by match_date ) B "
		        + " WHERE A.MATCH_ID = B.MATCH_ID and home_name is not null and away_name is not null AND MARKET_TYPE_ID in "
		        + " (SELECT splitdata FROM SplitString(@list,',')) ";
		// " AND B.live='T' and B.live_stat='1' AND B.visible='T' AND
		// B.booked='T' "+
		// " AND A.visible='T' AND A.suspend = 'F' ";
		// " and b.match_stat='0' "+add_query+" and b.match_date >= getdate()
		// order by match_date ";
		...
		String query3 = " SELECT '{\"Id\":'+CAST(A.MARKET_ID as varchar(10))+',\"MId\":'+CAST(B.MATCH_ID as varchar(10))+', "
		        ...
		        + " WHERE a.MATCH_ID = b.MATCH_ID " + // and a.market_id =
		                                              // b.market_id";
				// " and b.match_stat='0' "+add_query+" and b.match_date >=
				// getdate() order by match_date ";
		        " AND (MARKET_TYPE_ID ='377' or MARKET_TYPE_ID='399' or MARKET_TYPE_ID='2434' or MARKET_TYPE_ID='4475' "
		        + " OR MARKET_TYPE_ID='5488' OR MARKET_TYPE_ID='5498'   OR MARKET_TYPE_ID='8774' OR MARKET_TYPE_ID='6544' "
		        + " OR MARKET_TYPE_ID='6564' OR MARKET_TYPE_ID='8750' OR MARKET_TYPE_ID='8763' OR MARKET_TYPE_ID='9775' "
		        + " OR MARKET_TYPE_ID='10829' or MARKET_TYPE_ID='9321')";
		...
		String query4 = " SELECT '{\"Id\":\"'+CAST(C.SEL_ID as varchar(10))+'\",\"MId\":'+CAST(B.MATCH_ID as varchar(10))+',\"MkId\":\"'+CAST(C.Market_Id as varchar(10))+'\", "
		        ...
		        " from BC_MARKET A, (select top 10 * from BC_MATCH b where match_stat='0' " + add_query1
		        + " and match_date >= getdate() " + add_query2
		        + " and home_name is not null and away_name is not null order by match_date) b, bc_selection c " +
				// " WHERE B.live='T' and B.live_stat='1' AND B.visible='T' AND
				// B.booked='T' "+
				// " AND A.visible='T' and a.market_id = c.market_id and
				// b.market_id = c.market_id and home_name is not null and
		...
		try {
			...
			// System.out.println("getMainPreMatch : " + sb.toString());

		} catch (Exception e) {
			System.out.println("[getMainPreMatch] : " + e.getMessage());
	...
	public List<BettingListBean> getBetList(String mid) throws SQLException {
		...
		String query1 = " select bgid,bet_money,expect_rate,expect_money,sel_cnt,bgresult,convert(char(19),betdate,120) as betdate from bc_bet_grp where viewtype='Y' and userid='"
		        + mid + "' ";
		 ...
		 * " where a2.nameid = b2.sport_nameid "+ " group by txt,bgid";
		 */
		try {
	...
	public List<BettingSName> getBetSNames(String mid) throws SQLException {
		...
		String query1 = " select txt,bgid from  bc_trans a2, ( " + " select a1.sport_nameid,b1.* from bc_sport a1, ( "
		        + " select aaa.sport_id,bbb.* from bc_match aaa, (  " + " select aa.match_id,bb.* from bc_market aa, ( "
		        + " select b.bgid,market_id,b.sel_id  from bc_selection a,(select bgid,sel_id from bc_bet_sel where bgid in (select bgid from bc_bet_grp where userid='"
		        + mid + "' )) b "
		        + " where a.sel_id = b.sel_id ) bb where aa.market_id = bb.market_id ) bbb where aaa.match_id=bbb.match_id ) b1 "
		        + " where a1.sport_id = b1.sport_id ) b2 " + " where a2.nameid = b2.sport_nameid ";
		try {
	...
	public boolean detBetList(String[] bid, String st, String mid) throws SQLException {
		...
		// String query1 ="update bc_bet_grp set viewtype='N' where
		// userid='matthew' and bgid IN ? ";
		if (st.equals("SP")) {
		...
		try {
			...
			System.out.println("[detBetList] : " + e.getMessage());
	...
	public List<BetSelList> getBetDetail(String bid) throws SQLException {
		...
		query1 = " select * from ("
		        ...
		        + " select b.bgid,b.sel_id,bsresult,a.market_id,sel_name,sel_price  from bc_selection a,(select bgid,sel_id,bsresult from bc_bet_sel where bgid = '"
		        + bid + "') b  " + " where a.sel_id = b.sel_id) cc "
		        + " where aa.match_id=bb.match_id and bb.market_id = cc.market_id " + "  ) aaa,"
		        + " (SELECT SPORT_ID,CASE WHEN LEN(TXT) > 0 THEN TXT ELSE SPORT_NAME END  AS SNAME "
	...
	public List<BettingListBean_SC> getBetList_SC(String mid) throws SQLException {
		...
		String query1 = " select scid, siteid, userid, job, token, money, convert(char(19),regdate,120) regdate from spincube_lst where viewtype='Y' and userid='"
		        + mid + "'  order by regdate desc ";
		...
		try {
			...
			System.out.println("[getBetList_SC] : " + e.getMessage());
	...
	public List<BettingListBean_SC> getBetList_MG(String mid) throws SQLException {
		...
		String query1 = " select mgid, siteid, userid, job, money, convert(char(19),regdate,120) regdate from micro_game where viewtype='Y' and userid='"
		        + mid + "'  order by regdate desc ";
